...
from session_def import create_db_or_connect, Session, get_session
...
class TaskUpdate(SQLModel):
    ...
    description: Optional[str] = None
    progress_state: Optional[int] = None
    limit_at: Optional[datetime] = Field(
        default_factory=lambda: datetime.now(pytz.timezone("Asia/Tokyo")), nullable=True
    )
...
def get_task_or_exception(
    task_id: int, session: Session, status_code=status.HTTP_404_NOT_FOUND
):
    task_by_id = session.get(Task, task_id)
    if not task_by_id:
        raise HTTPException(
            status_code=status_code, detail=f"Task with id : {task_id} not found"
        )
    return task_by_id
...
def update(
    task_id: int, puted_task: TaskUpdate, session: Session = Depends(get_session)
):
